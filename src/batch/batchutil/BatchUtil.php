...
final class BatchUtil {
	...
	private static $stopWordArray = array('require','software developer','black','i','to','has','want','buy','sell','am','a','rs','have','had','will','would','shall','should','need', 'experience','with','for','hour','hr','required', 'having','color','colour','the', 'that', 'your', 'yours', 'aint','were','of','be','brand','new','product','products','service','services');
	...
	public static function getValueFromKey($key) {
		$value = ConfigXMLReader::getInstance('../batch/batchutil/configfile.xml')->getVlaueFromKey($key);
		return $value;
	}
	...
	public static function removeStopWordsImproved($string) {
		$logger = BatchUtil::getBatchLogger('BatchUtil.php');
		$stopWordArr = BatchUtil::$stopWordArray;
		$strInputArray = explode(" ", $string);
		foreach( $strInputArray as $key => $val ) {
			if( in_array($val, $stopWordArr) ) {
				unset($strInputArray[$key]);
			}
		}
		$string = implode(" ", $strInputArray);
		$cnt = count(BatchUtil::$gam);
		for($i = 0; $i < $cnt; $i++) {
			$string = str_replace(BatchUtil::$gam[$i], '', $string);
		}
		$string = BatchUtil::removeWhiteSpacesInBetweenString($string);
		$string=ltrim($string);
		$string=rtrim($string);
		$logger->debug("Changed String:: ".$string);
		return $string;
	...
	public static function removeWhiteSpacesInBetweenString($string) {

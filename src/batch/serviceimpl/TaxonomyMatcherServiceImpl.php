<?php
require_once ("../batch/service/TaxonomyMatcherService.php");
require_once ("../batch/serviceimpl/BatchDetailServiceImpl.php");
require_once ("../batch/batchdaoimpl/BatchDataDaoImpl.php");
	...
	public function mapWithTaxonomyTable($string,$taxonomyCategoryObjArray=null) {
		...
		if($string != ''){
			...
			if(null != $taxonomyCategoryObjArray && isset($taxonomyCategoryObjArray) && is_array($taxonomyCategoryObjArray)) {
				...
				foreach ($taxonomyCategoryObjArray as $catKey=>$catValue){
					//for($i = 0; $i < $iLen; $i++) {
					$product = $catValue->getProductIds();
					$cnt = count($product);
					...
					array_push($categoryIdArray, $catValue->getCategoryId());
				}
			}
			$productCategoryMapIds = $service->getProductCode($string, $categoryIdArray, $productCategoryMapIdAddary);
			...
			if($len == 0) {
				...
				if(null != $categoryIds && isset($categoryIds) && is_array($categoryIds)) {
					...
					for($j = 0; $j < $cnt; $j++) {
						$found = 'N';
						foreach ($taxonomyCategoryObjArray as $catKey=>$catValue) {
							if($categoryIds[$i] == $catValue->getCategoryId()) {
								$found = 'Y';
								break;
							}
						}
				...
				if($catLen > 0 && $iLen > 0) {
					foreach ($productCategoryMapIds as $productCategoryMapIdKey=>$productCategoryMapIdValue) {
						$prdCat = $productCategoryMapIdValue->getCategoryId();
						foreach ($taxonomyCategoryObjArray as $catKey=>$catValue){
							//for($j = 0; $j < $catLen; $j++) {
							$catCat = $catValue->getCategoryId();
							$this->logger->debug("Got Cat Id from Product:: ".$prdCat." Got Cat Id from Category:: ".$catCat);
							if(null == $catCat || $catCat == '') {
								$this->logger->debug("Unsetting Blank Category::");
								unset($taxonomyCategoryObjArray[$catKey]);
							}
							if($prdCat == $catCat) {
								//$this->logger->debug("Got Cat Id from Product:: ".$prdCat." Got Cat Id from Category:: ".$catCat);
				...
				$this->logger->debug("Final Count Of Category Array::" .count($taxonomyCategoryObjArray));
				//for($i = 0; $i < $len; $i++) {
				foreach ($productCategoryMapIds as $productCategoryMapIdKey=>$productCategoryMapIdValue) {
					$taxonomyCategoryObjArray = $productCategoryMapIdValue->formCategoriesFromProductCategory($taxonomyCategoryObjArray);
				}
			}
		}

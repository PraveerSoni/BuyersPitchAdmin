<?php
require_once ("../batch/service/BatchDetailService.php");
require_once ("../batch/batchdbfacadeimpl/BatchDetailDBFacadeImpl.php");
require_once ('../batch/batchdbfacadeimpl/BatchDataDaoFacadeImpl.php');
require_once ('../batch/batchdaoimpl/TaxonomyDaoImpl.php');
...
public function getProductCode($string,$categoryIdArray=null,$productCategoryMapIdArray = null) {
		$dao = new TaxonomyDaoImpl();
		$str = strtoupper($string);
		$prodIds = $dao->findProductMapForCode($str, $categoryIdArray, $productCategoryMapIdArray);
		if(count($prodIds) == 0) {
			$prodIds = $dao->findProductSynonnymMapForCode($str, $categoryIdArray, $productCategoryMapIdArray);
		}
		return $prodIds;
	}
	...
	public function getProductMap($string,$categoryIdArray=null,$productCategoryMapIdArray = null) {
		$dao = new TaxonomyDaoImpl();
		$str = strtoupper($string);
		$prodIds = $dao->findProductMapForBrand($str, $categoryIdArray, $productCategoryMapIdArray);
		if(count($prodIds) == 0) {
			$prodIds = $dao->findProductSynonnymMapForBrand($str, $categoryIdArray, $productCategoryMapIdArray);
			if(count($prodIds) == 0) {
				$prodIds = $dao->findProductMapForSubBrand($str, $categoryIdArray, $productCategoryMapIdArray);
				if(count($prodIds) == 0) {
					$prodIds = $dao->findProductSynonnymMapForSubBrand($str, $categoryIdArray, $productCategoryMapIdArray);
					if(count($prodIds) == 0) {
						$prodIds = $dao->findProductMap($str, $categoryIdArray, $productCategoryMapIdArray);
						if(count($prodIds) == 0) {
							$prodIds = $dao->findProductSynonnymMap($str, $categoryIdArray, $productCategoryMapIdArray);
						}
					}
				}
			}

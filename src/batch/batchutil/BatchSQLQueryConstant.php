...
final class BatchSQLQueryConstant {
	...
	const SQL_TAXONOMY_PROD_MAP = "select Product_Category_Map_Id, Category_Id from product_category_map where upper(Product_Name) = ?";
	
	const SQL_TAXONOMY_PROD_MAP_CODE = "select Product_Category_Map_Id, Category_Id from product_category_map where upper(product_code) = ?";
	
	const SQL_TAXONOMY_PROD_MAP_BRAND = "select Product_Category_Map_Id, Category_Id from product_category_map where upper(brand) = ?";
	
	const SQL_TAXONOMY_PROD_MAP_SUB_BRAND = "select Product_Category_Map_Id, Category_Id from product_category_map where upper(sub_brand) = ?";

	const SQL_TAXONOMY_CATEGORY = "select Category_Id from category where upper(Category_Name) = ? and Level <> ?";

	const SQL_TAXONOMY_CATEGORY_SYN = "select cs.Category_Id from categorysynonym cs, category c where upper(Synonym) = ? and c.Category_Id = cs.Category_Id and c.Level <> ?";
	...
	const SQL_TAXONOMY_PROD_SYN_MAP = "select ps.Product_Category_Map_Id, pcm.category_id from productsynonym ps, product_category_map pcm  where upper(Synonym) = ? and ps.Product_Category_Map_Id=pcm.Product_Category_Map_Id";
	
	const SQL_TAXONOMY_PROD_SYN_MAP_CODE = "select ps.Product_Category_Map_Id, pcm.category_id from productsynonym ps, product_category_map pcm  where upper(product_code_synonym) = ? and ps.Product_Category_Map_Id=pcm.Product_Category_Map_Id";
	
	const SQL_TAXONOMY_PROD_SYN_MAP_BRAND = "select ps.Product_Category_Map_Id, pcm.category_id from productsynonym ps, product_category_map pcm  where upper(brand_synonym) = ? and ps.Product_Category_Map_Id=pcm.Product_Category_Map_Id";
	
	const SQL_TAXONOMY_PROD_SYN_MAP_SUB_BRAND = "select ps.Product_Category_Map_Id, pcm.category_id from productsynonym ps, product_category_map pcm  where upper(sub_brand_synonym) = ? and ps.Product_Category_Map_Id=pcm.Product_Category_Map_Id";

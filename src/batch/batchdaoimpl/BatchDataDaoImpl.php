<?php
require_once ("../batch/batchdao/BatchDataDao.php");
require_once ("../domain/post/PostDetails.php");
require_once ("../domain/post/PostMapTaxonomy.php");
require_once ("../domain/batchdomain/NotifyBid.php");
...
class BatchDataDaoImpl extends AbstractPDODB implements BatchDataDao {
	...
	public function insertNonNotifyPost(PostDetailsDTO $postDetailsDto) {
		...
		$dbh = $this->formConnection();
		$postId = $postDetailsDto->getPostDetailsId();
		$postText = $postDetailsDto->getPostText();
		$dbh->beginTransaction();
		$stmt = $dbh->prepare($sql);
		$stmt->bindParam(1, $postId);
		$stmt->bindParam(2, $postText);
		$stmt->execute();
	...
	public function insertNonNotifyPostArray($postDetailsDtos) {
		...
		for($i = 0; $i < count($postDetailsDtos); $i++){
			$stmt = $dbh->prepare($sql);
			$postId = $postDetailsDtos[$i]->getPostDetailsId();
			$postText = $postDetailsDtos[$i]->getPostText();
			$stmt->bindParam(1, $postId);
			$stmt->bindParam(2, $postText);
			$stmt->execute();
			$id = $dbh->lastInsertId('Notify_Post_NonMatch_Id');

<?php
require_once ("../batch/strategy/AbstractBatchStrategy.php");
require_once ("../batch/strategy/BatchStrategy.php");
abstract class AbstractCustomeBatchStrategy extends AbstractBatchStrategy implements BatchStrategy {
	...
	public function executeStrategy() {
		...
		try{
			...
			if(strcasecmp($status, 'N') != 0) {
				throw new BatchJobException("Batch Job :: ".$this->batchDetail->getBatchType()->getBatchTypeName()." is alreday Started.", 4, null);
			}
			//if(strcasecmp($status, 'N') == 0) {
	...
	*/
	...
	protected final function removeStopWords($string) {
		...
		$string = BatchUtil::removeStopWordsImproved($string);
		return $string;
	}

<?php
require_once ("../domain/post/PostTypeMaster.php");
require_once ("../domain/userdetail/UserDetail.php");
require_once ("../domain/ImageDetail.php");
require_once ("../domain/post/ProductDetailDTO.php");
...
public function formCategoryProduct() {
		if(null != $this->categoryId) {
			...
			$i = 0;
			//for($i = 0; $i <$catCnt; $i++) {
			foreach ($this->categoryId as $catKey=>$catValue){
				$i = $i+1;
				//$category = $this->categoryId[$i];
				$this->catIds=$this->catIds.$catValue->getCategoryId();
				...
				if($prodCnt == 0) {
					...
					//for($j = 0; $j < $prodCnt; $j++) {
					foreach ($product as $prodKey=>$prodValue) {
						//$prod = $product[$j];
						$strPodId = $prodValue->getProductId();
